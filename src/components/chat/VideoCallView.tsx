...
export function VideoCallView({ callId, onEndCall, localUser, isAudioOnly }: VideoCallViewProps) {
  ...
  const [isCameraOff, setIsCameraOff] = useState(isAudioOnly); // Camera is off by default for audio-only
    ...
    if (callId && localUser) {
      ...
      try {
        ...
        if (callDocSnap.exists()) {
          ...
          // Delete the call document if the current user is the caller, or if it's an active call and current user is callee
          // More robustly, only delete if it's the fixed ID and call is ending, or if it's a dynamic ID call
          if (isLocalInitiated && (callData.callerId === localUser.uid || (callData.calleeId === localUser.uid && callData.status === 'active'))) {
            console.log("VideoCallView: Participant is ending call. Deleting call document and subcollections for callId:", callId);
          ...
          } else if (isLocalInitiated && callData.callerId !== localUser.uid && callData.calleeId !== localUser.uid) {
            // This case should ideally not happen if logic is correct, but handles if a non-participant tries cleanup somehow
            console.warn("VideoCallView: Non-participant initiated cleanup for callId:", callId);
          }
        }
      } catch (error) {
  ...
  useEffect(() => {
    console.log(`VideoCallView: useEffect for call initialization triggered. callId: ${callId}, localUser: ${localUser?.uid}, isAudioOnly: ${isAudioOnly}`);
    let pcInstance: RTCPeerConnection;
    let mediaAcquiredSuccessfully = false;
    let localIsCallerDetermination: boolean | null = null;
    ...
    const initialize = async () => {
      ...
      try {
        ...
        if (isAudioOnly) { // If audio only, ensure video tracks are off
        ...
        pcInstance.onicecandidate = event => {
          console.log("VideoCallView: onicecandidate event:", event?.candidate ? event.candidate : "No candidate", "Current localIsCallerDetermination:", localIsCallerDetermination);
          if (event.candidate && callId && localIsCallerDetermination !== null) { 
            const candidatesCollectionPath = localIsCallerDetermination ? `calls/${callId}/callerCandidates` : `calls/${callId}/calleeCandidates`;
        ...
        pcInstance.onconnectionstatechange = () => {
          ...
          if (pcInstance?.connectionState === 'connected') {
            ...
            setIsConnecting(false);
             ...
             if (!localUserInitiatedEndRef.current && (pcInstance?.connectionState === 'closed' || pcInstance?.connectionState === 'failed')) {
                 onEndCall(); 
             }
          }
        };
        ...
        pcInstance.onsignalingstatechange = () => {
            console.log("VideoCallView: Peer signaling state changed to:", pcInstance?.signalingState);
        };
        ...
        if (!callDocSnap.exists() || (callDocSnap.exists() && callDocSnap.data().status && ['ended', 'ended_by_caller', 'ended_by_callee'].includes(callDocSnap.data().status))) {
          if(callDocSnap.exists()){
            console.log("VideoCallView: Existing call document found but status is ended. Deleting and recreating as caller.", callDocSnap.data());
            const batch = writeBatch(firestore);
          ...
          console.log("VideoCallView: Offer created. Attempting to setLocalDescription. Signaling state:", pcInstance.signalingState);
          ...
          if (callData.isAudioOnly !== isAudioOnly) {
            toast({variant: "destructive", title: "Call Type Mismatch", description: `This call is ${callData.isAudioOnly ? 'audio-only' : 'video'}. You tried to join a ${isAudioOnly ? 'audio-only' : 'video'} call.`});
            onEndCall();
            return;
          }
          ...
          if (callData.callerId === localUser.uid) {
            ...
            console.log("VideoCallView: Callee: Answer created. Attempting setLocalDescription. Signaling state:", pcInstance.signalingState);
            ...
            console.warn("VideoCallView: Call document exists but state is unexpected or not joinable. Data:", callData);
        ...
        if (localIsCallerDetermination === null) {
            console.error("VideoCallView: CRITICAL - localIsCallerDetermination was not set.");
            toast({ variant: "destructive", title: "Internal Error", description: "Failed to determine call role." });
            onEndCall(); return;
        }

        const remoteCandidatesCollectionPath = localIsCallerDetermination ? `calls/${callId}/calleeCandidates` : `calls/${callId}/callerCandidates`;
            ...
            if (change.type === 'added') {
              ...
              try {
                if (peerConnectionRef.current && peerConnectionRef.current.signalingState !== 'closed') { 
                  await peerConnectionRef.current.addIceCandidate(new RTCIceCandidate(change.doc.data()));
        ...
        callDocUnsubscribeRef.current = onSnapshot(callDocRef, async (snapshot) => { 
          ...
          if (!pc || pc.signalingState === 'closed') {
            console.warn("VideoCallView: Call doc snapshot but PC is null/closed.");
            return;
          }

          if (localIsCallerDetermination === true && data?.answer && !pc.currentRemoteDescription) {
            console.log("VideoCallView: Caller: Detected answer. Signaling state:", pc.signalingState);
            if (pc.signalingState === 'have-local-offer' || pc.signalingState === 'stable') {
                 try {
                    await pc.setRemoteDescription(new RTCSessionDescription(data.answer));
                    console.log("VideoCallView: Caller: Remote description (answer) set. Signaling state:", pc.signalingState);
                    setIsConnecting(false); 
                } catch (e) {
                    console.error("VideoCallView: Caller: Error setting remote description (answer):", e, "Signaling state:", pc.signalingState);
                }
            }
          }

          if (data?.status && ['ended', 'ended_by_caller', 'ended_by_callee'].includes(data.status)) {
            const remotelyEndedByCaller = localIsCallerDetermination === false && data.status === 'ended_by_caller';
            const remotelyEndedByCallee = localIsCallerDetermination === true && data.status === 'ended_by_callee';
            
            if ((remotelyEndedByCaller || remotelyEndedByCallee || data.status === 'ended') && !localUserInitiatedEndRef.current) {
        ...
        console.error('VideoCallView: Error during initialize function:', error, (error as Error).stack);
        ...
        if (mediaAcquiredSuccessfully) {
          ...
          description = `Failed to set up the call. ${(error as Error).message || 'Please try again.'}`;
        } else {
    ...
    if (localUser?.uid) { 
        ...
        toast({ variant: "destructive", title: "User Error", description: "Your user information is not available."});
        onEndCall();
    }

    return () => {
      console.log("VideoCallView: useEffect cleanup triggered for callId:", callId);
      // Only call cleanupCall if this specific instance initiated a call that needs cleaning.
      // It will be called again by onEndCall if the parent triggers it.
      // This check prevents premature cleanup if the component unmounts unexpectedly.
      if (peerConnectionRef.current || callDocUnsubscribeRef.current || iceCandidateListenersUnsubscribeRef.current.length > 0) {
         cleanupCall(localUserInitiatedEndRef.current); 
      }
    };
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [callId, localUser?.uid, isAudioOnly]); // isAudioOnly is a dependency now
  ...
  const handleLocalEndCall = async () => {
        ...
        try {
            ...
            if (callDocSnap.exists()) {
                ...
                if (currentStatus !== 'ended' && currentStatus !== 'ended_by_caller' && currentStatus !== 'ended_by_callee') {
                    const roleAtSetup = isCaller; 
                    if (roleAtSetup === true) { // Explicitly check boolean true
                        await updateDoc(callDocRef, { status: 'ended_by_caller', endedAt: serverTimestamp() });
                    } else if (roleAtSetup === false) { // Explicitly check boolean false
                        await updateDoc(callDocRef, { 
                            status: 'ended_by_callee', 
                            calleeId: null, // Callee leaves, so calleeId becomes null
                            endedAt: serverTimestamp() 
                        });
                    }
                    console.log("VideoCallView: Updated call status to ended by current user for callId:", callId);
                } else if (currentStatus === 'ended_by_caller' && roleAtSetup === false) {
                    // If caller already ended, and callee clicks end, we can delete the doc
                    await cleanupCall(true); // This will delete the doc
                } else if (currentStatus === 'ended_by_callee' && roleAtSetup === true) {
                    // If callee already ended, and caller clicks end, we can delete the doc
                    await cleanupCall(true); // This will delete the doc
                }
            }
        } catch (error) {
    ...
    // onEndCall is the prop from ChatWindow, which will set isCallActive to false
    // cleanupCall will handle PC and stream cleanup
    onEndCall(); 
  };
  ...
  const toggleCamera = () => {
    if (isAudioOnly) return; // Camera toggle is disabled for audio-only calls
     setIsCameraOff(prev => {
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 w-full h-full flex-1">
        <div className="md:col-span-4 bg-muted rounded-lg overflow-hidden flex items-center justify-center relative aspect-video md:aspect-auto">
          ...
          {isAudioOnly && remoteStream && (
            <div className="flex flex-col items-center text-muted-foreground p-4 text-center">
                ...
                {/* Placeholder for audio visualization or more info */}
            </div>
          )}
          <video 
            ref={remoteVideoRef} 
            ...
            autoPlay 
            playsInline
            style={{ display: remoteStream && hasMediaPermission && !isAudioOnly ? 'block' : 'none' }}
          />
          {(!remoteStream && hasMediaPermission && !isAudioOnly) && (
            <div className="flex flex-col items-center text-muted-foreground p-4 text-center">
              {isConnecting ? (
                <>
           ...
           {hasMediaPermission && remoteStream && !isAudioOnly && <p className="absolute bottom-4 left-4 bg-black/50 text-white px-2 py-1 rounded text-sm">Remote User</p>}
        </div>
        ...
        <div className="md:col-span-1 flex flex-col gap-4">
                ...
                <>
                  ...
                  {(!localStream || isCameraOff || !hasMediaPermission ) && ( 
                     <div className="w-full h-full bg-muted flex items-center justify-center">
                        ...
                        {hasMediaPermission && (!localStream || (!isCameraOff && localStream?.getVideoTracks().every(t => !t.enabled))) && <UserIcon className="h-12 w-12 text-foreground/50" />}
                     </div>
                  )}
                </>
              )}
             {hasMediaPermission && localStream && !isAudioOnly && <p className="absolute bottom-2 left-2 bg-black/50 text-white px-1.5 py-0.5 rounded text-xs">
            ...
            <div className="hidden md:block p-2 bg-muted/50 rounded-lg text-center text-xs text-foreground/70 overflow-auto">
              ...
              <p>Status: {isConnecting && peerConnectionRef.current?.connectionState !== 'connected' ? "Connecting..." : (remoteStream ? "Connected" : (peerConnectionRef.current?.connectionState === 'closed' || peerConnectionRef.current?.connectionState === 'failed' ? "Ended" : "Waiting"))}</p>
      ...
      <Card className="absolute bottom-4 left-1/2 transform -translate-x-1/2 shadow-xl bg-card/80 backdrop-blur-sm z-20">
        <CardContent className="p-3 flex items-center gap-3">
          ...
          <Button 
            ...
            disabled={!localStream || hasMediaPermission === false || isAudioOnly}
