...
export function VideoCallView({ callId, onEndCall, localUser }: VideoCallViewProps) {
  ...
  const [hasMediaPermission, setHasMediaPermission] = useState<boolean | null>(null); // null initially, then true/false
    ...
    if (callId && localUser) {
      ...
      try {
        ...
        if (callDocSnap.exists()) {
          ...
          if (isLocalInitiated && (callData.callerId === localUser.uid || (callData.callerId && !callData.calleeId && callData.status !== 'active'))) {
  ...
  useEffect(() => {
    ...
    let localIsCallerDetermination: boolean | null = null; // Used for immediate logic before state updates
    ...
    const initialize = async () => {
      ...
      try {
        ...
        pcInstance.onicecandidate = event => {
          ...
          // Use localIsCallerDetermination as isCaller state might not be updated yet
          ...
          if (pcInstance?.connectionState === 'connected') {
            ...
            setIsConnecting(false);
             ...
             if (!callEndedByRemote && (pcInstance?.connectionState === 'closed' || pcInstance?.connectionState === 'failed')) {
                ...
                // onEndCall(); // This might be too aggressive.
             }
            setIsConnecting(true); // No longer actively trying to connect if failed/closed, show connecting or waiting
          }
        };
        ...
        if (!callDocSnap.exists() || (callDocSnap.exists() && callDocSnap.data().status && ['ended', 'ended_by_caller', 'ended_by_callee'].includes(callDocSnap.data().status))) {
          ...
          if (pcInstance.signalingState !== 'stable') {
            console.warn("VideoCallView: Caller: signalingState is not 'stable' before createOffer. Current state:", pcInstance.signalingState, "Attempting to create offer anyway.");
          }
          ...
          if (callData.callerId === localUser.uid) {
            ...
            if (callData.offer && !pcInstance.currentLocalDescription) {
              console.log("VideoCallView: Caller rejoining, setting local description from stored offer. Current signaling state:", pcInstance.signalingState);
              try {
                await pcInstance.setLocalDescription(new RTCSessionDescription(callData.offer));
                console.log("VideoCallView: Caller rejoining, local description set. New signaling state:", pcInstance.signalingState);
              } catch (e) {
            ...
            if (callData.answer && !pcInstance.currentRemoteDescription) {
              console.log("VideoCallView: Caller rejoining, setting remote description from stored answer. Current signaling state:", pcInstance.signalingState);
              try {
                await pcInstance.setRemoteDescription(new RTCSessionDescription(callData.answer));
                console.log("VideoCallView: Caller rejoining, remote description set. New signaling state:", pcInstance.signalingState);
              } catch (e) {
            ...
            console.log("VideoCallView: Callee: Setting remote description (offer) from callData.offer. Current signaling state:", pcInstance.signalingState);
            try {
              await pcInstance.setRemoteDescription(new RTCSessionDescription(callData.offer));
              console.log("VideoCallView: Callee: Remote description (offer) set. New signaling state:", pcInstance.signalingState);
            } catch (e) {
            ...
            try {
              ...
              console.log("VideoCallView: Callee: Local description (answer) set. New signaling state:", pcInstance.signalingState);
            } catch (e) {
        ...
        if (localIsCallerDetermination === null) {
            console.error("VideoCallView: CRITICAL - localIsCallerDetermination was not set. This should not happen.");
            ...
            if (change.type === 'added') {
              ...
              try {
                if (peerConnectionRef.current && peerConnectionRef.current.signalingState !== 'closed') { 
                  ...
                  console.warn("VideoCallView: PeerConnection closed or null, cannot add ICE candidate from Firestore.");
                }
              } catch (e) {
        ...
        callDocUnsubscribeRef.current = onSnapshot(callDocRef, async (snapshot) => { 
          ...
          if (!snapshot.exists()) {
            ...
            if (!callEndedByRemote && currentPcState !== 'closed' && currentPcState !== 'failed') {
                const isLocallyEnded = (peerConnectionRef.current?.signalingState === 'closed' && peerConnectionRef.current?.iceConnectionState === 'closed');
                 if (!isLocallyEnded) { // Avoid toast if local user initiated the end leading to deletion
          ...
          const pc = peerConnectionRef.current; // Use the ref for current PC instance
          if (!pc || pc.signalingState === 'closed') {
            console.warn("VideoCallView: Call doc snapshot received, but PC is null or closed. Aborting further processing.");
            return;
          }

          // Caller: if an answer appears, set it as remote description
          // Use localIsCallerDetermination here as well because isCaller state might not be updated yet
          if (localIsCallerDetermination === true && data?.answer && !pc.currentRemoteDescription) {
            console.log("VideoCallView: Caller: Detected answer in call document. Current Signaling state:", pc.signalingState);
            if (pc.signalingState === 'have-local-offer' || pc.signalingState === 'stable') { // stable if it was a renegotiation
                 try {
                    await pc.setRemoteDescription(new RTCSessionDescription(data.answer));
                    console.log("VideoCallView: Caller: Remote description (answer) set. New Signaling state:", pc.signalingState);
                    setIsConnecting(false); 
                } catch (e) {
                    console.error("VideoCallView: Caller: Error setting remote description (answer):", e, "Signaling state:", pc.signalingState);
                }
            } else {
                 console.warn("VideoCallView: Caller: Received answer but signalingState is not 'have-local-offer'. Current state:", pc.signalingState, "Answer:", data.answer);
            }
          }
          ...
          if (data?.status && ['ended', 'ended_by_caller', 'ended_by_callee'].includes(data.status)) {
            ...
            if (remotelyEndedByCaller || remotelyEndedByCallee || generallyEnded) {
                ...
                if (!callEndedByRemote && pc.connectionState !== 'closed') { // Avoid duplicate toasts
        ...
        try {
            ...
            if (callDocSnap.exists()) {
                ...
                if (currentStatus !== 'ended' && currentStatus !== 'ended_by_caller' && currentStatus !== 'ended_by_callee') {
                    // Use the determined role at the time of call setup
                    const roleAtSetup = isCaller; // isCaller state should be set by now from initialize
                    if (roleAtSetup) { 
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 w-full h-full flex-1">
            ...
            <div className="flex flex-col items-center text-muted-foreground p-4 text-center">
              {isConnecting ? (
                <>
                 ...
                 callEndedByRemote || peerConnectionRef.current?.connectionState === 'closed' || peerConnectionRef.current?.connectionState === 'failed' ? (
                    <>
        ...
        <div className="md:col-span-1 flex flex-col gap-4">
            ...
            <div className="hidden md:block p-2 bg-muted/50 rounded-lg text-center text-xs text-foreground/70 overflow-auto">
              ...
              <p>Status: {isConnecting && peerConnectionRef.current?.connectionState !== 'connected' ? "Connecting..." : (remoteStream ? "Connected" : (callEndedByRemote || peerConnectionRef.current?.connectionState === 'closed' || peerConnectionRef.current?.connectionState === 'failed' ? "Ended" : "Waiting"))}</p>
